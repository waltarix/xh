...
$(BINARY):
	RUSTFLAGS='--cfg reqwest_unstable' cargo build --locked --release --features=http3

$(MANUAL):
	OPENSSL_STATIC=1 cargo run --features=native-tls -- --generate man > $@

$(COMPLETIONS) &:
	OPENSSL_STATIC=1 cargo run --features=native-tls -- --generate complete-zsh > completions/_xh
	OPENSSL_STATIC=1 cargo run --features=native-tls -- --generate complete-bash > completions/xh.bash
	OPENSSL_STATIC=1 cargo run --features=native-tls -- --generate complete-fish > completions/xh.fish
	OPENSSL_STATIC=1 cargo run --features=native-tls -- --generate complete-elvish > completions/xh.elv
	OPENSSL_STATIC=1 cargo run --features=native-tls -- --generate complete-nushell > completions/xh.nu
